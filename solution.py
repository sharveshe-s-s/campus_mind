...
# ==========================================
# 2. HACKATHON WINNING CSS (REFINED)
# ==========================================
st.markdown("""
<style>
    ...
    .sidebar-title {
        font-weight: 700; font-size: 22px; letter-spacing: 0.08em; text-transform: uppercase; color: #E0FFEE;
    }
    .sidebar-subtitle {
        font-size: 11px; color: rgba(255,255,255,0.65); letter-spacing: 0.12em; text-transform: uppercase;
    }
    ...
    .hero-tagline { font-size: 16px; color: rgba(235, 241, 255, 0.85); }
    .hero-badge {
        display: inline-flex; align-items: center; gap: 6px; padding: 4px 10px;
        border-radius: 999px; background: linear-gradient(90deg, rgba(0, 200, 83, 0.18), rgba(0, 200, 150, 0.04));
        border: 1px solid rgba(0, 255, 140, 0.35); font-size: 11px; text-transform: uppercase;
        letter-spacing: 0.16em; color: #9fffdc; margin-bottom: 8px;
    }

    /* NAVIGATION PILLS */
    .nav-link {
        border-radius: 999px !important; margin: 4px 0 !important; padding: 0.5rem 0.85rem !important;
        font-size: 14px !important; font-weight: 500 !important; color: #d0d7ff !important;
        border: 1px solid transparent !important; transition: all 0.18s ease-out !important;
    }
    .nav-link:hover {
        background: radial-gradient(circle at top left, rgba(0, 200, 83, 0.18), rgba(0, 0, 0, 0.4));
        border-color: rgba(0, 255, 160, 0.28) !important; transform: translateX(2px);
    }
    .nav-link-selected {
        background: linear-gradient(120deg, #00C853, #009624) !important;
        color: #ffffff !important; box-shadow: 0 0 16px rgba(0, 200, 83, 0.55);
    }

    /* SEARCH BAR */
    .stTextInput input {
        background: rgba(4, 8, 20, 0.96) !important; color: #f1f5ff !important;
        border-radius: 999px; padding: 12px 18px 12px 46px; font-size: 16px;
    ...
    div[data-testid="stButton"] button {
        border-radius: 50% !important; width: 44px; height: 44px; padding: 0;
        background: radial-gradient(circle at 30% 20%, #00ffc6 0, #00C853 40%, #006622 85%);
        border: 1px solid rgba(0, 255, 170, 0.7); color: #02030a; font-size: 22px;
        transition: all 0.22s ease; box-shadow: 0 0 0 0 rgba(0, 255, 170, 0.5);
    }
    div[data-testid="stButton"] button:hover {
        transform: translateY(-1px) scale(1.05); box-shadow: 0 0 18px 4px rgba(0, 255, 170, 0.55);
    }
    ...
    .stButton button.process-btn {
        ...
        background-size: 220% auto;
        color: white; font-weight: 700; font-size: 15px; letter-spacing: 0.08em; text-transform: uppercase;
        box-shadow: 0 12px 30px rgba(0, 200, 83, 0.35); border: none;
    ...
    .glass-card {
        ...
        border: 1px solid rgba(255, 255, 255, 0.10); padding: 18px; margin-bottom: 15px;
        box-shadow: 0 18px 45px rgba(0, 0, 0, 0.9);
    }
    .answer-box-container {
        background: linear-gradient(135deg, rgba(0, 0, 0, 0.88), rgba(6, 20, 36, 0.96));
        border-radius: 18px; border: 1px solid rgba(0, 255, 170, 0.4);
        padding: 22px; margin-top: 24px; color: #ffffff !important;
        box-shadow: 0 24px 60px rgba(0, 0, 0, 0.9);
    }
    .answer-title { color: #9effc9; font-size: 18px; font-weight: 700; display: flex; align-items: center; gap: 6px; }
    ...
    .chip {
        ...
        border: 1px solid rgba(255,255,255,0.16); font-size: 11px; color: rgba(225,231,255,0.9);
        gap: 6px; background: rgba(10,16,32,0.9);
    }
...
# --- ASSETS (UPDATED WITH FUTURISTIC IMAGE) ---
# New "Futuristic AI/Robot" Animation
...
# Alternative Backup if that one doesn't load: "https://assets9.lottiefiles.com/packages/lf20_zrqthn6o.json"
...
if selected == "Student Chat":
    ...
    with col1:
        # Replaced with the Futuristic AI Lottie
    ...
    with col2:
        ...
        st.markdown("<h1 class='shimmer-text' style='font-size: 46px; margin-bottom: 6px;'>CampusMind AI</h1>", unsafe_allow_html=True)
        st.markdown("<p class='hero-tagline'>Ask about exams, circulars, or anything on campus — get instant, tailored answers in seconds.</p>", unsafe_allow_html=True)
        ...
        for i, file in enumerate(recent_files):
            with cols[i]:
                ...
                <div class="glass-card">
                    ...
                    <div style="color: #9fffdc; font-weight: 600; font-size: 14px; text-transform: uppercase; letter-spacing: 0.12em;">New Circular</div>
                    <div style="margin-top: 6px; font-size: 15px; color: #ffffff;">{file['name'][:40]}...</div>
                    <div style="font-size: 12px; color: #a3b3d6; margin-top: 8px;">🎧 Tap the mic and ask: <span style="font-style:italic;">“What is this circular about?”</span></div>
                </div>
                """, unsafe_allow_html=True)
    else:
    ...
    with right_col:
        ...
        if st.session_state.chat_history:
            ...
            st.markdown("<div style='font-size:12px;color:rgba(200,213,255,0.8);margin-top:4px;'>Start asking questions to see them appear here.</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
if selected == "Admin Portal":
    ...
    with c1:
        if lottie_admin: st_lottie(lottie_admin, height=160)
    with c2:
        st.title("Admin Upload")
        st.markdown('<p style="color:#c7d2ff;font-size:14px;">Securely upload circulars and instantly refresh what students can ask CampusMind.</p>', unsafe_allow_html=True)
